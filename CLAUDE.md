...
- **Next actions**: (1) Phase 1 frontend — slice picker + per-slice view consuming `data/slices/*.json` (generic scorer already in both `score_model.py` and the anchors-in-data design; respect `meta.coverage === 'insufficient'` and the §5.4 relative-vs-absolute framing: slice scores mean "vs this sector's own 2015-now history"); (2) remaining validation battery items (§7.2 vintage stability per slice as history accrues, §7.3 external corroboration); (3) Phase 2 occupation overlays — `IHLIDXUSTPPROJMANA` (Indeed PM postings) and `LNU04032215/6` are verified and ready.
